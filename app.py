...
if predict_button:
    ...
    if hasattr(model, "coef_"):
        ...
        # Extract feature names from preprocessor or fallback to input columns
        try:
            feature_names = preprocessor.get_feature_names_out()
        except AttributeError:
            feature_names = list(student_data.columns)

        # Convert sparse matrix or 2D array to a flat 1D numpy array
        if hasattr(student_processed, "toarray"):
            student_flat = student_processed.toarray().flatten()
        else:
            student_flat = student_processed.flatten()

        # Calculate directional impacts
        impacts = student_flat * coefficients
        ...
        # Clean up column prefixes generated by ColumnTransformer
        ...
        with col1:
            st.markdown(f"**Top Factors Supporting '{predicted_class}':**")
